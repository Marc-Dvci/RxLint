...
for cid in ids:
    c = demo.BY_ID[cid]
    d = Path(__file__).resolve().parents[1] / "fixtures" / "demo_cases" / cid
    blobs = {"rx": (d / "rx.jpg").read_bytes(), "label": (d / "label.jpg").read_bytes()}
    assets = [Asset.from_bytes("rx", "prescription", blobs["rx"], "image/jpeg"), Asset.from_bytes("label", "medicine", blobs["label"], "image/jpeg")]
